...
- Entry intro: `src/components/Intro.astro`. The A.A mark assembles out of the same 9.6 x 24 character cells the pointer reveal uses (positions snapped to that grid and de-duplicated), holds, then **collapses onto the wordmark** - the left A onto the A of "Adrian", the period onto the accent dot, the right A onto the A of "Antosiak" - fading out as the real letters rise through it. About 1.6s, then the canvas removes itself. An `is:inline` pre-paint script sets `--rise-base: 1.05s`, which every `.aa-rise` delay is offset by via `calc()`; with no JS or under `prefers-reduced-motion` the base stays `0s` and the copy appears at once. Four things it must keep doing, each of which cost an iteration:
  ...
  - The wordmark's resting box is `getBoundingClientRect()` minus its live transform. The reveal paints rather than moves today, so that transform is identity; the subtraction stays because an earlier translate-based reveal put a permanent 14px error into every landing.
...
- Motion otherwise: one entry reveal, and **every** element is in it - the wordmark, the services line, the contact links, and the chrome too (locale link, theme toggle, privacy link). `.aa-rise` (the name predates the effect) paints an element in from the left with `clip-path` in `steps(14, end)`, the same wipe the hover highlight uses, so the page has one motion language instead of two. Delays are inline, ordered content-then-chrome (0.16 / 0.28 / 0.40 / 0.52 / 0.58 / 0.64s), each offset by `--rise-base` - so until the monogram lands, the screen holds nothing but the monogram. Nothing else moves except the theme wipe and the pointer reveal.
  - The fill mode is **`backwards`, not `both`**, and that is load-bearing. `backwards` holds the clipped from-state through the delay so nothing flashes in early, and leaves no clip behind afterwards. With `both`, a stepped animation rests on its last step - one fourteenth still clipped - and the shortest labels lose their final character permanently.
  - `404.astro` runs the same reveal; with no intro on that page `--rise-base` falls back to `0s` and the sequence starts at once.
...
- Pointer reveal: `.aa-grid` is a fixed, inert `<canvas>` painted with leftovers of a terminal buffer - faint, meaningless punctuation (`. : - + / \\ | _ = * ' \``) sitting in the same 9.6 x 24px JetBrains Mono cells the intro assembles from, about half of them filled - shown only through a soft 190px radial `mask-image` that the pointer drags around. While the layer is visible, the script rewrites a few dozen random cells within 150px of the pointer every 70 ms, so the buffer keeps churning as if something were still computing; nothing outside that circle is ever repainted, and the loop stops on pointer leave. The glyph ink is the element's CSS `color` (`--color-grid`), read by the script, so it re-themes; the buffer is repainted on `document.fonts.ready`, on resize and on a `data-theme` change. Position is written as `--x` / `--y` custom properties on rAF, so a move costs one style write and no layout. The canvas is **created by the script**, not written in the markup: Astro's compiler moves a `<canvas>`'s closing tag past whatever follows it, which swallowed the entire page into the layer. It is gated on `(hover: hover)` and on `prefers-reduced-motion` being absent, so touch and motion-sensitive visitors never get the element at all; the CSS also hides it under reduced motion outright. The content wrapper carries `relative z-10` to sit above it. There is deliberately no readable text in the buffer - it must never grow into a fake prompt or command output. Earlier versions - a bare line grid, and before that a caret jumping between lines on hover - were rejected, and are not to come back.
...
- The one exception is the **end card**: the page does not scroll, so a scroll attempt (wheel down, swipe up, ArrowDown / PageDown / End / Space) swaps the copy block in `main` for a title card in the same spot - `Koniec.` / `The End.` in the wordmark's size with the accent period, one two-line sentence (`end_l` + `end_l2`, hard break between them), and the same two contact links. Header and footer do not move. Scroll up, ArrowUp / PageUp / Home or Escape swap back at once (`--rise-base` is zeroed on the home block so the copy does not wait for an intro that already ran). The card opens with `.aa-iris`, a circle growing out of the block, and its lines paint in with `.aa-rise`. A 700 ms lock stops one wheel gesture from toggling twice. It is a joke about the page ending, not a second page: no new copy goes on it.
