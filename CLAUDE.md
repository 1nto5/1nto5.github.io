...
- Motion: one entry reveal. `.aa-rise` (global.css) with per-element `animation-delay` (0.1 / 0.22 / 0.34s), no-op under `prefers-reduced-motion`. Nothing else moves except the theme wipe.
...
- `public/favicon.svg`, `public/favicon.ico` (32x32), `public/apple-touch-icon.png` (180x180), `public/og.png` + `og-en.png` (1200x630 share cards). All carry the site palette: Mocha ground, `#cdd6f4` marks, and the A.A monogram's square period in the accent. The share card is the page itself - mono name, the services line, the accent cursor beam.
